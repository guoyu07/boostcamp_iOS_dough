...
class PlayViewController: UIViewController {
	...
	private func alertWhenClear() {
		...
		let okAction = UIAlertAction(title: "OK", style: .default, handler: { (action) -> Void in
			...
			self.record.userName = stringFromTextField
			self.record.recordDate = self.convertTimeIntervalToKSTDate(timeInterval: self.finishTime)
			if let stringFromTimerLabel = self.timerLabel.text {
				self.record.timeRecord = stringFromTimerLabel
			}
	...
	private func convertTimeIntervalToKSTDate(timeInterval: TimeInterval) -> String {
